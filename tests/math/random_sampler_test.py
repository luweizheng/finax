...
import finax.math.random_sampler as random_sampler
...
class OpsTest(jtu.JaxTestCase):
    ...
    def test_shapes(self):
        ...
        sample_no_batch = random_sampler.normal_pseudo([2, 4], mean=jnp.array([0.2, 0.1]), key=subkey)
        ...
        sample_no_batch = random_sampler.normal_pseudo([2, 4], mean=jnp.array([0.2, 0.1]), key=subkey)
        print(sample_no_batch)
        sample_batch = random_sampler.normal_pseudo([2, 4], mean=jnp.array([[0.2, 0.1], [0., -0.1], [0., 0.1]]), key=subkey)
