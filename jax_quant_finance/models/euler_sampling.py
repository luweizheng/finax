...
def sample(
    ...
    if batch_size > 1:
        sample_fn = vmap(partial(_sample, dim=dim, drift_fn=drift_fn, volatility_fn=volatility_fn,
                times=times, keep_mask=keep_mask, num_requested_times=num_requested_times,
            num_samples=num_samples,
    ...
    #     normal_draws = jnp.asarray(normal_draws, dtype=dtype)
        # (time_steps, num_samples, dim)
        # num_samples = normal_draws.shape[-2]
...
def _sample(*,
    ...
    print(f"[num_samples, dim]: {num_samples}, {dim}")
    ...
    if normal_draws is None:
        ...
        #     random_sampler.RandomType.SOBOL,
        #     random_sampler.RandomType.HALTON,
        #     random_sampler.RandomType.HALTON_RANDOMIZED,
        ...
        #         random_type=random_type, dtype=dtype, seed=seed)
        #     wiener_mean = None
        # else:
    ...
    if loop_method == 'while':
        while_loop_jit = jit(_while_loop, static_argnames=['drift_fn', 'volatility_fn', 'dtype', 'random_type'])
        return while_loop_jit(steps_num=steps_num,
...
def _while_loop(*, steps_num, current_state,
    ...
    if isinstance(num_requested_times, int) and num_requested_times == 1:
        ...
        record_samples = True
        element_shape = current_state.shape
        # The following line could not be jit compiled
        # result = jnp.zeros((num_requested_times, ) + element_shape, dtype=dtype)
    ...
    #     return jnp.expand_dims(result, axis=-2)
    # Shape [num_time_points] + batch_shape + [num_samples, dim]
    # print(f"euler sampling: {result.shape}")
    # result = result.stack()
...
def _euler_step(*, i, written_count, current_state,
                ...
                random_type=None, normal_draws, result,
                record_samples=None, prng_key):
    ...
    if normal_draws is not None:
        ...
        new_key, key = random.split(prng_key)
        dw = random_sampler.normal_pseudo(
            (num_samples,), mean=wiener_mean, key=key)
    dw = dw * sqrt_dt[i]
